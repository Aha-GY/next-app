...
const DetailCard: React.FC<JobDescriptionProps> = ({
  description,
  responsibilities,
  idealCandidate,
  whenAndWhere,
  ...
  const formatDate = (post: any) => {
    ...
    const [datePart, timePart] = post?.split("T");
      ...
      <div className="text-[#25324B] pt-4 w-[815px]">
        ...
        <div className="mb-12">
          ...
          <ul>
            {responsibilities
              ?.split("\n")
              .map((responsible: string, index: number) => (
              ...
              .map((trait: string, index: number) => {
                ...
                return (
                  <li key={index} className="ml-7 mb-3">
                    <p className="font-semibold">{first}</p> {second}
                  </li>
                );
              })}
          </ul>
      ...
      <div className="w-[293.5px]">
          ...
          <div>
            <div className="flex flex-row mb-4">
              ...
              <div>
                ...
                <p className="font-medium"> {formatDate(datePosted)}</p>
              </div>
            </div>
        ...
        <div>
          ...
          <ul className="flex flex-row">
            {categories?.map((curr: string, index: number) => (
              <li key={index}>
                {index % 2 === 1 && (
                  <div className="font-semibold  bg-[#56CDAD1A]  rounded-full px-3 py-1 text-[#56CDAD] mr-3 text-xs w-fit h-fit">
                    {curr}
                  </div>
                )}

                {index % 2 === 0 && (
                  <div className="font-semibold  bg-[#EB8533] bg-opacity-10  rounded-full px-3 py-1 text-[#FFB836] mr-3 text-xs  w-fit h-fit">
                    {curr}
                  </div>
                )}
              </li>
  ...
  // return (
  //   <div className="w-[1229px] min-h-[1064px] pt-8 gap-16 opacity-100 flex flex-col">
  //     {/* First Component */}
  //     <div className="w-[815px] py-11 gap-14 opacity-100 flex flex-col bg-white shadow-lg rounded-lg">
  //       <div className="p-8">
  ...
  //       <div className="p-8">
  //         <h2 className="text-2xl font-bold mb-4">Ideal Candidate</h2>
  //         {idealCandidate && idealCandidate.length > 0 ? (
  //           <ul className="list-disc list-inside">
  //             {idealCandidate?.split("\n").map((item, index) => (
  //               <li key={index}>{item}</li>
  //             ))}
  //           </ul>
  //         ) : (
  //           <p>No ideal candidate information available.</p>
  //         )}
  //       </div>

  //       <div className="p-8">
  //         <h2 className="text-2xl font-bold mb-4">When & Where</h2>
  //         <p>{whenAndWhere || 'No details available.'}</p>
  //       </div>
  //     </div>

  //     {/* Second Component */}
  //     <div className="w-auto min-h-[674px] gap-5 opacity-100 flex flex-col bg-white shadow-lg rounded-lg p-8">
  //       <div className="flex flex-col">
  //         <h2 className="text-xl font-bold mb-2">About</h2>
  //         <p><strong>Posted On:</strong> {postedOn || 'N/A'}</p>
  ...
  //         <p>{categories && categories.length > 0 ? categories.join(', ') : 'No categories available.'}</p>
  //       </div>

  //       <div className="flex flex-col">
  //         <h2 className="text-xl font-bold mb-2">Required Skills</h2>
  //         <p>{requiredSkills && requiredSkills.length > 0 ? requiredSkills.join(', ') : 'No skills listed.'}</p>
  //       </div>
  //     </div>
  //   </div>
  // );
